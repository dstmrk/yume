...
- A flap is a card with two halves, and it turns through the drum of its characters.
  `SplitFlapCell` gives the two halves and one card for each character of the drum. A
  surface that turns one time is not a Solari. Paragraph 5.6 of `docs/architecture.md`
  gives the rules.
