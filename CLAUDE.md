...
- A flap is a card with two halves, and it turns through the drum. The drum holds the
  separator of the thousands, then the digits. It holds no letter: the name of the
  application does not turn. `SplitFlapCell` gives the two halves and one card for each
  digit of the drum. A surface that turns one time is not a Solari. Paragraph 5.6 of
